"""Production HTTP and AI resource limits for Image Studio.

Layers
------
1. Per-IP sliding windows on heavy routes (abuse / DoS)
2. Global AI job queue (one heavy job at a time by default)
...
Env (all optional)
------------------
IMAGE_STUDIO_RATE_LIMIT_ENABLED=1
IMAGE_STUDIO_TRUST_PROXY=0
IMAGE_STUDIO_RATE_LIMIT_AI=8/minute
IMAGE_STUDIO_RATE_LIMIT_HEAVY=3/minute
IMAGE_STUDIO_RATE_LIMIT_OUTPUT=12/minute
IMAGE_STUDIO_RATE_LIMIT_POST=60/minute
IMAGE_STUDIO_AI_MAX_CONCURRENT=1
IMAGE_STUDIO_AI_QUEUE_WAIT_S=120
IMAGE_STUDIO_MIN_FREE_RAM_GIB=3.0
IMAGE_STUDIO_MIN_FREE_VRAM_GIB=0.35
IMAGE_STUDIO_UNLOAD_MODELS=1
IMAGE_STUDIO_AI_COOLDOWN_<ROUTE>=seconds   e.g. UPSCALE, DETECT
IMAGE_STUDIO_RAM_RESERVE_<ROUTE>_GIB=…    override per-route RAM floor
"""
...
RATE_LIMIT_ENABLED = _env_bool("IMAGE_STUDIO_RATE_LIMIT_ENABLED", True)
TRUST_PROXY = _env_bool("IMAGE_STUDIO_TRUST_PROXY", False)

_AI_COUNT, _AI_WINDOW = _parse_limit(
    os.environ.get("IMAGE_STUDIO_RATE_LIMIT_AI", "8/minute"), 8, 60.0
)
_HEAVY_COUNT, _HEAVY_WINDOW = _parse_limit(
    os.environ.get("IMAGE_STUDIO_RATE_LIMIT_HEAVY", "3/minute"), 3, 60.0
)
_OUTPUT_COUNT, _OUTPUT_WINDOW = _parse_limit(
    os.environ.get("IMAGE_STUDIO_RATE_LIMIT_OUTPUT", "12/minute"), 12, 60.0
)
_POST_COUNT, _POST_WINDOW = _parse_limit(
    os.environ.get("IMAGE_STUDIO_RATE_LIMIT_POST", "60/minute"), 60, 60.0
)
...
_OUTPUT_PATHS = {"/api/optimize-png"}
...
def _cooldown(route: str) -> float:
    env_key = f"IMAGE_STUDIO_AI_COOLDOWN_{route.upper()}"
...
def rate_limit_status() -> dict[str, object]:
    return {
        ...
        "windows": {
            ...
            "output": {"limit": _OUTPUT_COUNT, "window_s": _OUTPUT_WINDOW},
...
async def _hit_window(
    ...
    *,
    ...
    now = time.monotonic()
    # The ASGI app is exercised by more than one event loop in tests and some
    # embedded hosts. A short process lock is safe here because this section has
    # no I/O or awaits, and unlike asyncio.Lock it is not bound to one loop.
    with _windows_lock:
...
class SecurityRateLimitMiddleware(BaseHTTPMiddleware):
    """Per-IP sliding-window limits for write, AI, and output routes."""
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        lookup = path if path in _AI_PATHS or path in _OUTPUT_PATHS else path
        ...
        elif lookup in _OUTPUT_PATHS or path in _OUTPUT_PATHS:
            checks.append(
                ("output", _OUTPUT_COUNT, _OUTPUT_WINDOW, "Output rate limit exceeded")
            )
...
async def acquire_ai_slot(route: str) -> AsyncIterator[None]:
    """Queue for a global inference slot; gate on free memory; cleanup after.

    Waiters sit in an asyncio queue (semaphore) up to ``IMAGE_STUDIO_AI_QUEUE_WAIT_S``.
    ...
    try:
        ...
        try:
            ...
            try:
                _last_cleanup = await run_blocking(release_inference_memory)
