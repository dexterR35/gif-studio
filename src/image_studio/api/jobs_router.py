"""Versioned routes for creating, inspecting, and cancelling background jobs."""
...
async def create_job(body: JobCreate, request: Request) -> JobStatus | Any:
...
async def get_job(job_id: str, request: Request) -> JobStatus | Any:
...
async def cancel_job(job_id: str, request: Request) -> JobStatus | Any:
...
async def get_job_result(job_id: str, request: Request) -> dict[str, Any] | Any:
    record = job_store.get(job_id)
    if record is None:
        return problem_response(
            status=404,
