"""Image Studio RAM/VRAM checks and post-inference cleanup.

Used by the AI job queue so heavy work such as upscale and detection only
...
def min_free_ram_bytes() -> int:
    ...
    return max(256 * 1024 * 1024, int(_env_float("IMAGE_STUDIO_MIN_FREE_RAM_GIB", 3.0) * _GIB))


def route_reserve_bytes(route: str) -> int:
    env_key = f"IMAGE_STUDIO_RAM_RESERVE_{route.upper()}_GIB"
...
def unload_models_enabled() -> bool:
    ...
    return _env_bool("IMAGE_STUDIO_UNLOAD_MODELS", True)
