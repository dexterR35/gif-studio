...
export function ProjectAside() {
  ...
  const preview = hasSource ? (
    <>
      <div className="relative mb-2 aspect-[1.55] overflow-hidden rounded-[10px] bg-surface checker">
        ...
        {canReplace && (
          <div className="absolute inset-0 grid place-items-center bg-black/55 opacity-0 group-hover:opacity-100">
  ...
  return (
    <aside
      className={`scrollbar absolute inset-y-0 left-0 z-20 h-full w-[286px] overflow-y-auto overscroll-contain border-r border-white/[.06] bg-panel px-3.5 lg:relative lg:inset-auto lg:shrink-0 lg:translate-x-0 ${mobilePanel ? 'translate-x-0' : '-translate-x-full'}`}
    >
      ...
      <div className="py-3">
        {canReplace ? (
          <button
            ...
            className={cn(
              'focus-ring group w-full rounded-[12px] border border-dashed p-2.5 text-left',
